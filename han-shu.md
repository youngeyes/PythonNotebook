...
### return语句
return语句表示函数结束，不带return语句的函数，解释器会认为返回None。函数执行只要碰到return语句则表示函数结束。例如：<br/>
```
def sum(arg1,arg2):
    total = arg1 + arg2
    return total
...
### 变量的作用域
变量是有其作用域的，一个程序的所有变量不是在哪个位置都能访问的。比如函数体内部的变量在函数外面是访问不到的。所以有变量的作用域产生，有两种基本的变量作用域：
- 全局变量
- 局部变量

#### 全局变量和局部变量
全局变量是定义在整个程序中的，整个程序范围都可以访问到，而所有声明在函数内部的变量都是局部变量。实例如下：<br/>
```
total = 0 # 这是一个全局变量
def sum(arg1,arg2):
    total = arg1 + arg2 # 此处的total是局部变量
...
全局变量total的值： 0
```
以上实例可以看出，局部变量如果跟全局变量名一样的话，它不是引用全局变量，而是创建一个变量名跟全局变量名一样的局部变量。局部作用域内可以访问全局的变量，但是不能对全局变量进行修改，如果要修改全局变量，需要用到golbal关键字。例如：<br/>
```
total = 0 # 这是一个全局变量
def sum(arg1,arg2):
    global total # 此处的total是全部变量
    total = arg1 + arg2 
    print("变量total的值：",total)
    return total
...
全局变量total的值： 30
```
> Tips:变量名解析的机制为：LEGB法则。
> - L:本地作用域
> - E:上一层结构作用域
> - G:全局作用域
> - B:内置作用域

> **LEGB作用域查找原则**：当引用一个变量时，Python解释器先从本地作用域开始查找，如果没找到，则去上一层作用域查找，再到全局作用域找，最后在内置作用域找，层层查找，直到找到。若找不到则报错。
